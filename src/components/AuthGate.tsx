...
export default function AuthGate({ children }: { children: React.ReactNode }) {
  ...
  function validate(needsName = false) {
    ...
    if (view !== "forgot" && password.length < 6) { setFeedback("A senha precisa ter pelo menos 6 caracteres."); return false; }
  ...
  return (
    <AuthLayout>
      ...
      {!isForgot ? <View style={styles.socialRow}><Pressable disabled={submitting} onPress={handleGoogle} accessibilityLabel="Continuar com Google" style={styles.socialButton}><GoogleLogo /></Pressable><Text style={styles.socialHint}>Entrar com Google</Text><Text style={styles.comingSoon}>Outros em breve</Text></View> : null}
...
function Field(props: { value: string; onChangeText: (text: string) => void; placeholder: string; secureTextEntry?: boolean; keyboardType?: "email-address" }) { return <TextInput value={props.value} onChangeText={props.onChangeText} placeholder={props.placeholder} placeholderTextColor="#8A8799" autoCapitalize="none" keyboardType={props.keyboardType} secureTextEntry={props.secureTextEntry} style={styles.input} />; }
...
const styles = {
  ...
  socialRow: { flexDirection: "row", alignItems: "center", justifyContent: "center", marginTop: 20 },
  ...
  comingSoon: { color: "#9B98A8", fontSize: 11, marginLeft: "auto" },
